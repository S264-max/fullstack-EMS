...
const EmployeeCard = ({employee,onDelete, onEdit}) => {
  ...
  return (
    <div className='group relative card card-hover overflow-hidden'>
        ...
        {!employee.isDeleted && (
            <div className='absolute inset-0 bg-linear-to-t from-indigo-700/20 via-transparent to-transparent opacity-0 group-hover:opacity-100
            transition-opacity flex items-end justify-center pb-6 gap-3'>
                <button onClick={()=>onEdit(employee)} className='p-2.5 bg-white/90 backdrop-blur-sm text-slate-700 hover:text-indigo-600 rounded-xl shadow-lg
